...
import com.neocean.mylibrary.R
...
open class BottomBehavior constructor(context: Context, attrs: AttributeSet) :
    ...
    override fun onDependentViewChanged(parent: CoordinatorLayout, child: BottomNavigation, dependency: View): Boolean {
        if (isSnackbar(dependency)) {
            if (null == snackbarDependentView) {
                snackbarDependentView = SnackBarDependentView(dependency as Snackbar.SnackbarLayout, height, bottomInset)
            }
    ...
    class SnackBarDependentView internal constructor(child: Snackbar.SnackbarLayout, height: Int,
                                                     bottomInset: Int) : DependentView<Snackbar.SnackbarLayout>(child, height, bottomInset) {
